...
function Tab:CreateSlider(SliderSettings)
	...
	-- Function to update slider value and display
	local function UpdateSliderValue(NewValue)
		-- Clamp value to range
		NewValue = math.max(SliderSettings.Range[1], math.min(SliderSettings.Range[2], NewValue))
		...
		TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.45, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Size = UDim2.new(0, Slider.Main.AbsoluteSize.X * ((NewValue - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])) > 5 and Slider.Main.AbsoluteSize.X * ((NewValue - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])) or 5, 1, 0)
		}):Play()
	...
	-- Input field functionality for clicking on the information text
	Slider.Main.Information.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 then
			-- Create a temporary TextBox for input
			local InputBox = Instance.new("TextBox")
			InputBox.Size = Slider.Main.Information.Size
			InputBox.Position = Slider.Main.Information.Position
			InputBox.BackgroundColor3 = SelectedTheme.TextboxBackground or Color3.fromRGB(30, 30, 30)
			InputBox.BackgroundTransparency = 0.2
			InputBox.BorderSizePixel = 1
			InputBox.BorderColor3 = SelectedTheme.TextboxStroke or Color3.fromRGB(100, 100, 100)
			InputBox.TextColor3 = SelectedTheme.TextColor or Color3.fromRGB(255, 255, 255)
			InputBox.TextScaled = true
			InputBox.Font = Slider.Main.Information.Font
			InputBox.PlaceholderText = "Enter value..."
			InputBox.Text = tostring(SliderSettings.CurrentValue)
			InputBox.Parent = Slider.Main
			InputBox.ZIndex = 10
			
			-- Hide the original information text
			Slider.Main.Information.Visible = false
			
			-- Focus the input box
			InputBox:CaptureFocus()
			InputBox:SelectAll()
			
			-- Function to finish input
			local function FinishInput()
				...
				-- Restore original information text
				Slider.Main.Information.Visible = true
				InputBox:Destroy()
			end
